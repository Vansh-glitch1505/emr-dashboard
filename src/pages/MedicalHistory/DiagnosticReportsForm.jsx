...
export default function DiagnosticReportsForm({ closeForm, onSave }) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (finalList.length === 0) {
        alert('Please add at least one diagnostic report');
        return;
      }
      ...
      for (const report of finalList) {
        if (report.file) {
          ...
          const response = await fetch('http://localhost:5000/api/medical-history/diagnostic-reports', {
            method: 'POST',
            body: formData
          });
          ...
          if (response.ok) {
            ...
            throw new Error(errorData.message || 'Failed to upload diagnostic report');
          }
        }
      }
      ...
      alert("Diagnostic reports saved successfully!");
      console.log("Saved diagnostic reports:", uploadedReports);
    } catch (error) {
      console.error("Error saving diagnostic reports:", error);
      alert(`Error saving diagnostic reports: ${error.message}`);
    }
  };
    ...
    return (
      <div className="right-panel">
        <div className="form-header">
          <h2 className="form-title">Diagnostic Reports Preview</h2>
          ...
          {savedData && savedData.length > 0 ? (
            savedData.map((report, index) => (
              <div key={report.id} className="immunization-preview-item">
                <h4>Diagnostic Report {index + 1}</h4>
            ...
            <p>No diagnostic reports added.</p>
          )}
        </div>
    ...
    <div className="right-panel">
      ...
      {reportList.length > 0 && (
        <div className="added-immunizations">
          <h3>Added Diagnostic Reports:</h3>
      ...
      <div className="form-group">
        ...
        <textarea
          ...
          placeholder="Enter comments about the diagnostic report (e.g., X-ray results, MRI findings, ultrasound notes)"
        />
      </div>
